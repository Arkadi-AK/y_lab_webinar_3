...
@router.post(
    path="/logout_all",
    summary="Завершить все другие сеансы",
    tags=["users"]
)
def logout_all(user_service: UserService = Depends(get_user_service),
    ...
    return {"msg": "Вы вышли из системы со всех устройств."}
...
def refresh_token(user_service: UserService = Depends(get_user_service),
    ...
    try:
        ...
        raise HTTPException(
            status_code=HTTP_403_FORBIDDEN, detail="Не удалось проверить учетные данные"
        )
...
def user_create(
    ...
    """Registers a user"""
    response = {"msg": "Пользователь создан успешно."}
